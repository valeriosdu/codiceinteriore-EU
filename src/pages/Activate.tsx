  ...
  const handleSSOLogin = async (provider: "google" | "apple") => {
    ...
    try {
      const result = await lovable.auth.signInWithOAuth(provider, {
        redirect_uri: window.location.origin + "/activate" + window.location.search,
      });
      ...
      if (result.redirected) {
        // Browser will redirect to provider; nothing else to do here.
        return;
      }

      // Tokens were returned inline (no redirect). The auth listener will
      // pick up the SIGNED_IN event and call saveProfileAndRedirect.
      // Fallback: if listener didn't fire yet, trigger redirect manually.
      const { data: { user } } = await supabase.auth.getUser();
      if (user) {
        await saveProfileAndRedirect(user.id);
      } else {
        setSsoLoading(null);
      }
    } catch (err) {
