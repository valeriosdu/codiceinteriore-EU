  ...
  for (const queue of ['auth_emails', 'transactional_emails']) {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      try {
        ...
        // accurate. Fall back to an insert if no pending row exists (shouldn't
        // happen in normal flow, but keeps the log self-healing).
        {
          const { data: updated, error: updateErr } = await supabase
            ...
            .select('id')
          if (updateErr) {
            console.error('Failed to update pending row to sent', {
              message_id: payload.message_id,
              error: updateErr,
            })
          }
