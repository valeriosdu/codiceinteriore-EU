-- Correzione della 20260605120000 (merge clienti).
--
-- Due problemi della prima versione:
--   1. ROTTURA: admin_customer_detail chiamava public.resolve_email_key(...),
--      funzione introdotta da una migration del repo (20260527200000) MAI
--      applicata al DB live → "function public.resolve_email_key(text) does not
--      exist" a runtime sulla scheda cliente. Qui rimuoviamo quella riga: il
--      resto del corpo è completo e corretto.
--   2. PRESTAZIONI: la view e l'helper chiamavano funzioni per-riga
--      (admin_primary_email / admin_member_emails con = ANY(array)) → niente
--      indici sulla lista. Qui le riscriviamo in forma set-based / con
--      uguaglianza scalare indicizzata. A tabella admin_customer_merges vuota il
--      comportamento è identico alle versioni originali, ma veloce.
--
-- NON tocca: admin_customer_merges, admin_merge_customers, admin_unmerge_customer
-- (repoint dei soli link di proprietà), admin_customers_search (invariata),
-- frontend.

-- ============================================================================
-- 1. admin_quiz_sessions_for_email — fold via LATERAL, uguaglianza scalare
-- ============================================================================
-- Per ogni email del gruppo (1 sola se non unita) esegue i 4 rami originali con
-- "= m.email": Postgres può usare gli indici su lower(customer_email)/lower(email).
...
-- ============================================================================
-- 2. admin_customer_index — base veloce (originale) + fold set-based
-- ============================================================================
-- base = corpo della 20260521190000 (GROUP BY lower(email), usa gli indici).
-- Poi un solo LEFT JOIN alla tabella minuscola admin_customer_merges ri-chiave
-- ogni riga alla principale e ri-aggrega. A merges vuota è identica all'originale.
...
SELECT
  ...
  -- preferisci i dati della riga principale (m NULL = non è una secondaria)
...
-- ============================================================================
-- 3. admin_customer_detail — rimossa la dipendenza da resolve_email_key
-- ============================================================================
-- Identica alla 20260605120000 SENZA la riga
...
-- ============================================================================
-- 4. Reload schema cache di PostgREST
-- ============================================================================
