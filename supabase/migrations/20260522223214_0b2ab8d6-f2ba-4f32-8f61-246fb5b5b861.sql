...
-- Synastry sessions table
CREATE TABLE IF NOT EXISTS public.synastry_sessions (
...
CREATE INDEX IF NOT EXISTS synastry_sessions_funnel_created_idx
  ON public.synastry_sessions (funnel_slug, created_at DESC);
CREATE INDEX IF NOT EXISTS synastry_sessions_status_idx
...
DROP TRIGGER IF EXISTS synastry_sessions_updated_at ON public.synastry_sessions;
...
DROP POLICY IF EXISTS "Service role update synastry" ON public.synastry_sessions;
...
-- Extend admin_customer_detail with synastry_sessions
