-- Estende admin_customer_detail per includere le synastry_sessions del cliente.
--
-- Le synastry_sessions sono collegate al cliente via checkout_sessions.customer_email
-- + checkout_sessions.synastry_session_id (vedi migration 20260522130000_synastry_sessions.sql).
--
-- Nuovo campo nel JSON di output: "synastry_reports" come array.
...
COMMENT ON FUNCTION public.admin_customer_detail IS
  'Dettaglio singolo cliente per /admin/clienti/:email — include synastry_sessions (acquisti coppia) accanto a quiz_sessions natale, checkouts, transit cycles e subscription.';
