...
--
-- L'admin attuale è quiz_session-centric: una persona compare in più righe se
-- ha più sessioni, e gli orfani (checkout pagati senza account) vivono in una
-- pagina separata. La nuova area è CRM-first: 1 riga = 1 email, aggrega
-- profilo + tutti i checkout (anche orfani) + tutti i quiz_sessions associati
-- + transiti + feedback.
--
-- Identità cliente = lower(email). Le uniche tabelle con email sono profiles
-- (utenti registrati) e checkout_sessions (chiunque abbia pagato, anche
-- orfani). Gli altri dati si appendono via profile_id o quiz_session_id.
--
-- Contenuto:
--   1. pg_trgm + indici trigram su email/nome per ricerca ILIKE veloce.
--   2. Helper SQL admin_quiz_sessions_for_email(text): set di quiz_session_id
--      raggiungibili da un'email (via checkout, profilo diretto, user_reports).
--      Usata sia dalla list che dalla detail per evitare duplicare la UNION.
--   3. View admin_customer_index: rollup base per email da profiles +
--      checkout_sessions. Non materializzata: dopo un recovery orfano la lista
--      è subito aggiornata.
--   4. RPC admin_customers_search: paginazione + filtri + sort + lateral
--      enrichment per la pagina visibile. Ritorna { total, page, page_size,
--      customers[] }.
--   5. RPC admin_customer_detail: dettaglio completo di un singolo cliente in
--      una sola query (profile + quiz_sessions + checkouts + transit_cycles +
--      transit_subscription + feedback + contacts_log). Sostituisce ~8
--      round-trip dell'edge function originale con una sola chiamata.
--
-- View e RPC sono accessibili solo da service_role: le edge functions
-- admin-customers-list / admin-customer-detail usano la service role dopo aver
-- verificato il header x-admin-secret.
...
-- Equality / UNION su lower(email). idx_checkout_sessions_customer_email
-- esiste già dalla 20260423.
CREATE INDEX IF NOT EXISTS idx_profiles_email_lower
  ON public.profiles (lower(email));

-- Trigram per ILIKE '%substr%' su email e nome.
...
-- ============================================================================
-- 2. Helper: quiz_session_id raggiungibili da un'email
-- ============================================================================
-- Tre rotte per legare un quiz_session a un'email (la tabella non ha colonna
-- email):
--   a) checkout_sessions.customer_email → checkout_sessions.quiz_session_id
--   b) profiles.email → profiles.quiz_session_id (link diretto)
--   c) profiles.email → user_reports.profile_id → user_reports.quiz_session_id
...
-- ============================================================================
-- 3. View admin_customer_index
-- ============================================================================
-- 1 riga = 1 email_key. Solo le aggregazioni puramente per-email (profilo +
-- checkout) stanno qui. Quelle che attraversano quiz_session si calcolano
-- nella RPC, lateral, solo per la pagina visibile.
...
-- ============================================================================
-- 5. RPC admin_customer_detail
-- ============================================================================
-- Dettaglio completo di un cliente in una sola query. Sostituisce la sequenza
-- di 8 round-trip dell'edge function admin-customer-detail con una RPC unica.
